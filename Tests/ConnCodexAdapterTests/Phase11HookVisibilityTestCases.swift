...
enum Phase11HookVisibilityTestCases {
    ...
    private static func malformedAndUnknownHookFactsAreIsolated(
        ...
        do {
            ...
            suite.check(false, "unknown hook status raised unexpected error \(error)")
        }
        do {
            ...
            suite.check(false, "malformed hooks/list raised unexpected error \(error)")
        }
        do {
            ...
            suite.check(false, "malformed hook run raised unexpected error \(error)")
        }
    }
